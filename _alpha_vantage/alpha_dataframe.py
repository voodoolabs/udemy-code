...
def rsi_dataframe(stock=stock):
    ...
    data_ts = ts.get_intraday(
        stock.upper(), interval='1min', outputsize='full')

    ti = TechIndicators(key=stock.upper(), output_format='pandas')

    data_ti, meta_data_ti = ti.get_bbands(symbol=stock.upper(
    ...
    while buy_point != len(buy) and sell_point != len(sell):
        if buy_index[buy_point] < sell_index[sell_point]:
            buy_positions += round(float(buy[buy_point]))
            print(
                f'buy position = {buy[buy_point]} total positions = {round(buy_positions, 2)} at sell index = {sell_index[sell_point]}')
            buy_point += 1
            stocks += 1
        else:
