...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Input(id='wait-time-box', value='', type='number',
                  placeholder='Wait Time Min.', ),
        dcc.Input(id='stocks-number-box', value='',
                  type='number', placeholder='Stocks Per Buy', ),
    ...
    dcc.Tabs(id='tabs', value='graph-1', children=[
        dcc.Tab(label='Bollinger Band', value='graph-1', children=[
            ...
            html.Div(dcc.Graph(id='candle-graph', animate=True,
                               style={"backgroundColor": "#1a2d46", 'color': '#ffffff'},)),
            html.Div(id='table-div')
        ]),
        dcc.Tab(label='RSI', value='graph-2', children=[
            html.Div(
                dcc.Graph(id='candle-graph2', animate=True,
                          style={"backgroundColor": "#1a2d46", 'color': '#ffffff'},)
            )
        ])
    ]),
...
def update_layout(n_clicks, tab, input_value, input_wait, input_stocks):
    ...
    data_ti2, meta_data_ti2 = ti.get_bbands(symbol=input_value.upper(
    ), interval='1min', time_period=period, series_type='close')
    ...
    for bl, p, i in zip(bb_low, low, total_df.index[::-1]):
        if p < bl:
            if not buy_index:
                ...
                index_need_to_beat = buy_index[-1] + \
                    datetime.timedelta(minutes=input_wait)
    ...
    for bh, p, i in zip(bb_high, high, total_df.index[::-1]):
        if p < bh:
            if not sell_index:
                ...
                index_need_to_beat = sell_index[-1] + \
                    datetime.timedelta(minutes=input_wait)
                if i > index_need_to_beat:
                    sell.append(p)
                    sell_index.append(i)
    ...
    while buy_point != len(buy):
        if buy_index[buy_point] < sell_index[sell_point]:
            ...
            values_df_prep['Sell Price'].append(
                round(sell[sell_point] * stocks, 2))
    ...
    for value, time, r in zip(total_df['RSI'], total_df.index, rsi_offset):
        if value > 60:
            if not sell_rsi_time:
                ...
                index_need_to_beat = sell_rsi_time[-1] + \
                    datetime.timedelta(minutes=input_wait)
    ...
    for value, time, r in zip(total_df['RSI'], total_df.index, rsi_offset):
        if value < 35:
            if not buy_rsi_time:
                ...
                index_need_to_beat = buy_rsi_time[-1] + \
                    datetime.timedelta(minutes=input_wait)
                if time > index_need_to_beat:
                    buy_rsi_value.append(r)
                    sell_rsi_time.append(time)
